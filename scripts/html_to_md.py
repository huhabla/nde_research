...
def html_to_markdown(html_path, css_selector, relevance_hint, check_css):
...
def main(input_file, css_selector, relevance_hint, check_css):
    ...
    for i, html_file in enumerate(tqdm(html_files, total=total_files, unit="file")):
        ...
        markdown_content = html_to_markdown(html_path, css_selector, relevance_hint, check_css)
        ...
        tqdm.write(f"Converted {i + 1}/{total_files}: {html_file} to {md_file}")
        converted_count += 1
